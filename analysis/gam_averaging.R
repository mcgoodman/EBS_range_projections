...
for (i in 1:length(roms_files)) {
  ...
  if (grepl("hindcast", roms_files[i])) {
    
    yr_sub <- which(format(roms_yrs, "%Y") %in% levels(model_data$year_chr))
    roms <- roms |> slice(yr_sub, along = "ocean_time")
    roms_yrs <- roms_yrs[yr_sub]
  ...
  for (j in 1:length(roms_yrs)) {
    ...
    if (grepl("hindcast", roms_files[i])) {
      ...
      exclude <- NULL
      roms_yr_df$year_chr <- format(roms_yrs[j], "%Y")
    ...
    binom_fit <- simplify2array(lapply(binom_models, \(x) predict(x, newdata = roms_yr_df[roms_keep,], type = "response", exclude = exclude, newdata.guaranteed = TRUE)))
    binom_se <- simplify2array(lapply(binom_models, \(x) predict(x, newdata = roms_yr_df[roms_keep,], type = "response", exclude = exclude, newdata.guaranteed = TRUE, se.fit = TRUE)$se.fit))
    ...
    tw_fit <- simplify2array(lapply(tw_models, \(x) predict(x, newdata = roms_yr_df[roms_keep,], type = "response", exclude = exclude, newdata.guaranteed = TRUE)))
    tw_se <- simplify2array(lapply(tw_models, \(x) predict(x, newdata = roms_yr_df[roms_keep,], type = "response", exclude = exclude, newdata.guaranteed = TRUE, se.fit = TRUE)$se.fit))
